...
YUI().use('node', 'overlay', 'io-form', function (Y) {
	...
	Y.io(BASE_URL + '/people/partial.php?partial=people/searchForm.inc', {
		on: {
	...
	Y.on('click', Y.bind(overlay.hide, overlay), '#find_person_overlay .cancel button');

	Y.on('submit', function (e) {
		e.preventDefault();
		Y.io(BASE_URL + '/people/partial.php?partial=people/searchResults.inc', {
			form: { id: e.target },
			on: {
				complete: function (id, o, args) {
