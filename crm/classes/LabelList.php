 ...
 */
class LabelList extends MongoResultIterator
{
	/**
	 * @param array $fields
	 */
	...
	public function find($fields=null,$order=array('name'=>1))
	{
		...
		if (count($search)) {
			$this->cursor = $this->mongo->labels->find($search);
		}
		else {
			$this->cursor = $this->mongo->labels->find();
		}
		if ($order) {
	...
	/**
	 * Hydrates all the Label objects from a database result set
	 *
