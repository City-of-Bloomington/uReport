<?php
/**
 * A collection class for ActionType objects
 *
 ...
 */
class ActionTypeList extends ZendDbResultIterator
{
	/**
	...
	{
		$this->select->from('actionTypes');

		// Finding on fields from the actionTypes table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the actionTypes table.
	...
	/**
	 * Hydrates all the ActionType objects from a database result set
	 *
	...
	{
		return new ActionType($this->result[$key]);
	}
}
