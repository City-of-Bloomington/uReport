<?php
/**
 * A collection class for Issue objects
 *
 ...
 */
class IssueList extends ZendDbResultIterator
{
	/**
	 ...
	 */
	public function find($fields=null,$order='date desc',$limit=null,$groupBy=null)
	{
		$this->select->from('issues');

		// Finding on fields from the issues table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the issues table.
	...
	/**
	 * Hydrates all the Issue objects from a database result set
	 *
	...
	{
		return new Issue($this->result[$key]);
	}
}
