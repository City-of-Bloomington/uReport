<?php
/**
 * A collection class for CategoryNote objects
 *
 ...
 */
class CategoryNoteList extends ZendDbResultIterator
{
	/**
	...
	{
		$this->select->from('category_notes');

		// Finding on fields from the category_notes table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the category_notes table.
	...
	/**
	 * Hydrates all the CategoryNote objects from a database result set
	 *
	...
	{
		return new CategoryNote($this->result[$key]);
	}
}
