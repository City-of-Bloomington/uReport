...
class ActionType
{
	...
	public function __construct($id=null)
	{
		if ($id) {
			if (is_array($id)) {
				...
				$sql = ctype_digit($id)
					? 'select * from actionTypes where id=?'
					: 'select * from actionTypes where name=?';
			...
			if ($result) {
				...
				throw new Exception('actionTypes/unknownActionType');
			}
		}
		else {
	...
	{
		$zend_db = Database::getConnection();
		$zend_db->update('actionTypes',$data,"id='{$this->id}'");
	}

	private function insert($data)
	{
		$zend_db = Database::getConnection();
		$zend_db->insert('actionTypes',$data);
		$this->id = $zend_db->lastInsertId('actionTypes','id');
	}
