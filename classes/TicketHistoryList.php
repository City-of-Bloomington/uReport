<?php
/**
 * A collection class for TicketHistory objects
 *
 ...
 */
class TicketHistoryList extends ZendDbResultIterator
{
	/**
	...
	{
		$this->select->from('ticketHistory');

		// Finding on fields from the ticketHistory table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the ticketHistory table.
	...
	/**
	 * Hydrates all the TicketHistory objects from a database result set
	 *
	...
	{
		return new TicketHistory($this->result[$key]);
	}
}
