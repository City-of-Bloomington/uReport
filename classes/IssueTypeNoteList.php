<?php
/**
 * A collection class for IssueTypeNote objects
 *
 ...
 */
class IssueTypeNoteList extends ZendDbResultIterator
{
	/**
	...
	{
		$this->select->from('issueType_notes');

		// Finding on fields from the issueType_notes table is handled here
		if (count($fields)) {
		...
		// above foreach only handles fields from the issueType_notes table.
	...
	/**
	 * Hydrates all the IssueTypeNote objects from a database result set
	 *
	 * This is a callback function, called from ZendDbResultIterator.  It is
	 * called once per row of the result.
	 *
	 * @param int $key The index of the result row to load
	 * @return IssueTypeNote
	 */
	protected function loadResult($key)
	{
		return new IssueTypeNote($this->result[$key]);
	}
}
